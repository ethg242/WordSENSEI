...
# algB: Variant of algA using tf - Term Frequency
def algB(context, word):
	...
	for senseURI in senseURIs:
		...
		#     *signifying no keywords or just no matches; either way tf should be 0 as per above
		if weight != 0:
			tf = weight / float(totalKeywords)
		
		log('Finished checking weights.', 4)
		
		# Catalogue the sense:weight
		wsenses.append( (senseName, tf, senseURI) )
		
		log('Done checking sense at weight %.4f (%d/%d).' %(tf, weight, totalKeywords), 3)
